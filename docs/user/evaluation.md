...
**Status:** unsigned alpha evaluation; this institutional source snapshot is published.
Core source `2c2ff27` and the separately tested Windows package `10a1300` have
distinct provenance in [SOURCE.md](../../SOURCE.md). No new binary package, tag or
GitHub Release accompanies this source update. Use the exact artifact hashes from
your trusted delivery record; a public source checkout is not a tested binary.
...
For a separately delivered original Core archive, the manifest is
`OPEN_SOURCE_EXPORT_MANIFEST.json`; its inventory includes
file paths, lengths and hashes. From the extracted Core root, verify the inventory
against the approved commit. This command applies to the original Core export;
the curated institutional repository has its own [distribution inventory](../../DISTRIBUTION_MANIFEST.json)
and does not contain the original export's metadata:
