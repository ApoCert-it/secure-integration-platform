...
provider, the .NET SDK and the Admin UI/API. The software is a technical
...
[![SIP architecture: an installed Windows application uses the SDK and Local Broker, which connects to the Gateway through mTLS and BGW1. Local keys use DPAPI protection; server-side execution uses Published configuration, provider capabilities and restricted egress. PostgreSQL and the Admin control plane support the Gateway.](docs/images/sip-architecture-overview.png)](docs/images/sip-architecture-overview.png)

The diagram shows the Windows Broker path. A Direct client can use the same Gateway
protocol without the Broker; standalone local protection needs no Gateway at all.
DPAPI `CurrentUser` wraps local data keys and small secrets under the service
identity. AES-256-GCM protects application data, while a separate non-exportable
CNG key identifies the Installation. These are distinct from vendor credentials
and signing keys held server-side through the configured provider.

Local protection does not hide plaintext returned to an authorized application or
defeat Administrator/SYSTEM. FSE2 is an optional integration example, not a Core
dependency or a claim that every illustrated path is live-qualified. See the
[Local Broker guide](docs/user/local-broker.md) and
[security model](docs/security/security-model.md) for the exact boundaries.
...
The [capability summary](IMPLEMENTATION_STATUS.md) describes integrated capabilities
and the qualification limits of each surface. These paths are
not interchangeable:
...
- **Windows / Local Broker.** The [Local Broker guide](docs/user/local-broker.md)
  describes standalone protection, service delivery, application credential adoption
  and Broker → Gateway evaluation. Real-service and standard-account observations
  apply to the specific software and environments identified there, not every
...
The separate Core-only export excludes deployment providers and vertical packs.
[Export boundaries](OPEN_SOURCE_BOUNDARIES.md) and [architecture](ARCHITECTURE.md)
explain the separation. This institutional source snapshot also includes the public
FSE2, Azure and local PKCS#12 packs; they remain optional and outside the Core.
